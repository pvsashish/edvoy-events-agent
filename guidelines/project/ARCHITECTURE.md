...
Server-side source of truth for the sidebar **API Usage** card (was localStorage-only, which reset on a browser cache clear). `api/usage.js`: GET reads it; POST `{delta}` atomically increments (one generate); POST `{reset:true}` zeroes it; POST `{set}` overwrites (used to restore a known value — e.g. the 2026-07-23 correction below). The client loads it on mount and keeps localStorage as an offline cache. **The card itself no longer exposes a Reset control** (removed 2026-07-23, UI-only — the endpoint's `{reset:true}` path is untouched) **or an input/output ratio bar** (also removed — it was read as a usage-limit gauge, but nothing in this app has a token/cost limit). `input_tokens`/`output_tokens`/`cost_usd` were corrected 2026-07-23 (**179,297 / 22,807 / $0.88**): the row had been seeded 2026-07-07 from an admitted estimate, and no Anthropic Admin API key exists in this project to pull real historical usage for an exact reconciliation, so the fix instead anchored `cost_usd` to the user's verified real spend and rescaled tokens proportionally against confirmed Sonnet 4.6 pricing ($3/M in, $15/M out) so the two stay internally consistent.
...
- **Hover-capability gating (2026-07-22)**: Scout's event rows attach `onMouseEnter`/`onMouseLeave` (background highlight + copy-icon reveal + image prefetch) only when `window.matchMedia('(hover: hover)').matches` — checked once at mount into a `scoutSupportsHover` state. Previously always attached, which is invisible on desktop but breaks touch: iOS Safari requires a first tap to "reveal" any element with hover-triggered visual changes before a second tap can fire its click, so every Scout event needed two taps to select on a real iPhone. `cursor: pointer` and an empty `onTouchStart` do **not** fix this — the trigger is the hover-reveal pattern itself, not a missing touch signal or an ambiguous click target. General lesson for this codebase: any future `onMouseEnter`/`onMouseLeave` pair that toggles visible UI (not just a CSS `:hover` cursor change) needs the same `matchMedia('(hover: hover)')` guard, or it will silently break single-tap selection on touch devices while looking completely correct in every desktop-based test (including this project's own Chromium-based QA browser, which always reports `hover: hover` regardless of viewport size).
