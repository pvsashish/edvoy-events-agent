...
No open blockers. DB lives in its own dedicated Neon project (`edvoy-events-agent`, not shared with any other tool). Scout images are on Cloudflare R2, not Neon — the old base64-in-Postgres transfer problem is permanently gone. The whole app sits behind a shared-password login gate (2026-07-16). Scout's canvas + the whole app's responsive layout was overhauled 2026-07-21, and a follow-up iOS-only interaction bug in the same area was fixed 2026-07-22 — both were real-device-only bugs invisible to desktop testing and to a Chromium-based simulated browser. The sidebar API Usage card's cumulative cost/token figures were corrected and simplified 2026-07-23 (see below).

### Recent (2026-07-23, latest) — API Usage card: corrected cumulative cost/tokens + dropped the ratio bar (data fix + shipped + live)
User flagged the sidebar's cumulative `$0.9175` figure as wrong — their real tracked spend was `$0.88`. Traced it to the `edvoy_usage` DB row's origin: it was seeded 2026-07-07 from a **known estimate** (29 generations / ~118k tokens / $0.60, explicitly logged then as "estimates — only the cost was known"), and every real per-generate delta added since inherited that estimate's error.
- **No Anthropic Admin API key exists in this project** (only a regular `sk-ant-api03-...` key), so the real historical usage couldn't be pulled programmatically from Anthropic's Usage & Cost API, and no per-call usage logging exists server-side to reconstruct it from logs either — confirmed both dead ends before proceeding.
- **Verified Sonnet 4.6 pricing is correct as coded** ($3/M input, $15/M output, `api/analyze.js`) via Anthropic's public pricing docs — so the formula wasn't the bug, the stored token counts were.
- **Fix:** `cost_usd` set to the user's verified real figure (`0.88`); `input_tokens`/`output_tokens` then proportionally rescaled (same ~88.7%/11.3% ratio preserved) so `tokens × real pricing = $0.88` exactly — keeps cost and tokens internally consistent instead of two disagreeing numbers. Final: **179,297 in / 22,807 out = 202,104 tokens, $0.88, 35 generations** (generations left untouched — never disputed). Applied via the app's own `POST /api/usage {set}` endpoint (not a raw DB write) through the authenticated browser session.
- **UI simplified** (`public/app.jsx`): removed the purple input/output-split progress bar under the cost figure — user pointed out it reads as a usage-limit gauge (implying output tokens are capped) when no such limit exists anywhere in the app; it was purely an input/output ratio visual. Also removed the "Cumulative" word from the card subtitle (now just "Sonnet 4.6") and removed the manual **Reset** button (the `POST /api/usage {reset:true}` endpoint itself is untouched and still callable directly if ever needed).
- **Cost display precision** also changed from 4 decimals (`$0.9175`) to 2 (`$0.88`) to match how a dollar figure is normally read; the separate per-generate `$cost` chip (results header) keeps its existing 4-decimal format, unchanged.
- **QA:** zero console errors across all 4 tabs (Event Generator, Specs History, Naming Converter, Scout), zero regression at 375px (mobile drawer) and 1280px (desktop) — card renders cleanly at both, no dangling references to the removed bar/reset code.
- **Note:** local `vercel dev` and Vercel prod share the same Neon `DATABASE_URL` in this project (as always) — this was a direct data correction, so it was live the instant it was applied; no separate "push to prod data" step existed.
- **Shipped:** commit `2580dbc` on `main` → pushed → Vercel auto-deploys.
...
- **API Usage counter is now DB-persisted** (`edvoy_usage` table + `api/usage.js`) — survives cache clears, consistent across devices; localStorage is just an offline cache. Was localStorage-only. Sidebar card simplified 2026-07-23 (see Current State above) — just cost/generates/tokens now, no ratio bar or manual Reset.
